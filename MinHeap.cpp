...
// @author: Robert Guy
//
// This header file represents a binary min-heap data structure.
...
// for every node X (excluding the root) the parent has a smaller key
	...
	void PercolateDown(size_t index)
	{
		...
		if (GetLeftChildNode(index) < _size && GetRightChildNode(index) < _size)
		{
			...
			if (_cache[left_child] > -1 && _cache[right_child] > -1)
			{
				...
				const unsigned int smallest_value = min(_cache[left_child], _cache[right_child]);
				const size_t smallest_index = smallest_value == _cache[left_child] ? left_child : right_child;
				swap(_cache[parent], _cache[smallest_index]);
				PercolateDown(smallest_index);
...
public:
	...
	friend ostream& operator<<(ostream& out, const MinHeap& min_heap)
	{
